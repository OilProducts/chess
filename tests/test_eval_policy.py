...
from chessref.eval.eval_policy import EvalConfig, InferenceConfig, PolicyEvalResult, RuntimeConfig, evaluate_policy
from chessref.train.train_supervised import DataConfig, ModelConfig
...
def test_evaluate_policy_smoke(tmp_path: Path) -> None:
    pgn_path = _write_sample_pgn(tmp_path)

    model_cfg = ModelConfig(num_moves=4672, d_model=64, nhead=4, depth=1, dim_feedforward=128, dropout=0.0, use_act=False)
    data_cfg = DataConfig(
    ...
    inference_cfg = InferenceConfig(max_loops=2, min_loops=1, use_act=False)
    runtime_cfg = RuntimeConfig(batch_size=2, device="cpu")
    ...
    assert result.cross_entropy == result.cross_entropy
